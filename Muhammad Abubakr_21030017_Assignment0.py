...
b = np.array([[1,2,4,3],[4,5,4,6]])    # Create a rank 2 array
...
print('Given number is: ' + checkParity(10));
print('Given number is: ' + checkParity(7));
...
for string in x:
    if len(string) % 2 == 0:
        print(string + ' length: ' + str(len(string)) + ', Parity: Even')
    else:
        print(string + ' length: ' + str(len(string)) + ', Parity: Odd')
...
def getResult(ids, maths_scores):
    resultList = {studentId: [ids[studentId], maths_scores[studentId]] for studentId in ids }
    return resultList

print(getResult(ids, maths_scores))
...
def printResult(string):
    ...
    for letter in string:
        if(letter.isupper()):
            ...
            lowerCaseCount += 1
    print('Total upper case count: ' , upperCaseCount)
    print('Total upper case count: ' , lowerCaseCount)
...
print('Two numbers from given list that can add up to k:' , k , ' are:' , calculateSum(l,k))
...
def find_students(address, students):
  ...
  for student in students:
    if(students.get(student).get("address") == address):
...
students = {
      ...
      "Zafar": {"age": 6, "address": "Lahore"},
      "Yahya": {"age": 8, "address": "Lahore"},
...
class evenNumbers:
  ...
  def next(self):
    return [x for x in range(1, self.n + 1) if x % 2 == 0]
    
myrange = evenNumbers(20)
print (evenNumbers.next(myrange))
...
class TupleInfo:
    ...
    def setTuple1(self, tuple1):
        self.tuple1 = tuple1
    
    def getTuple1(self):
        return self.tuple1
    
    def setTuple2(self, tuple2):
        self.tuple2 = tuple2
    ...
    def addition(self):
        return sum(self.tuple1)
    
    def subtraction(self):
        return self.tuple1[0] - self.tuple1[1]
...
tupleInfo = TupleInfo()
tupleInfo.setTuple1(tuple1)
tupleInfo.setTuple2(tuple2)

print(tupleInfo.addition())
print(tupleInfo.subtraction())
print(tupleInfo.multiplication())
print(tupleInfo.division())
print(tupleInfo.power())
...
plt.subplot(3, 1, 2)
y1 = (np.sin(x) * np.sin(x)) * (np.sin(x) * np.sin(x) * np.sin(x))

plt.plot(x, y1, color='green',linewidth=15)
plt.xlabel('x axis')
plt.ylabel('(Sin x)^2 * (Sin x)^3')
plt.title('Sin Func Plot')
plt.legend(['Y1'], loc ="upper center")


# SubPlot 3
plt.subplot(3, 1, 3)
y1 = np.square(np.cos(x))

plt.plot(x, y1, color='green', marker='o', linewidth=0, markerfacecolor='none', markeredgewidth=1)
plt.xlabel('x axis')
plt.ylabel('(Cos x)^2')
plt.title('Cos Func Plot')
...
studentData = pd.read_csv('/home/abu-bakr/Documents/ML@LUMS/students_data.csv')
studentNames = studentData['Name']
mathScores = studentData['Math']
readingScores = studentData['Reading']
writingScores = studentData['Writing']

plt.xlabel('Students')
plt.ylabel('Scores')
plt.title('Math BAR Chart')
plt.bar(studentNames, mathScores)
plt.show() 

plt.xlabel('Students')
plt.ylabel('Scores')
plt.title('Reading BAR Chart')
plt.bar(studentNames, readingScores)
plt.show() 

plt.xlabel('Students')
plt.ylabel('Scores')
plt.title('Writing BAR Chart')
plt.bar(studentNames, writingScores)
plt.show() 
...
import re as re
data = pd.read_csv('/home/abu-bakr/Documents/ML@LUMS/Twitter_US_Airline_Sentiment.csv', usecols = ['text'])
twitterHandlesSet = set()

def getTwitterHandlesSet(data):
    global twitterHandlesSet
    x = re.findall('@[A-Za-z0-9]+', data.to_string(index=False))
    twitterHandlesSet = set(x)

getTwitterHandlesSet(data)
print(len(twitterHandlesSet))
print(twitterHandlesSet)
...
### write code here...
cleanedTweets = []
cleanedTweets += [re.sub('[,?!.;:]','', str(x)) for x in data['text']]
print(cleanedTweets)
...
def getFilteredWords(data):
    global x
    x = re.findall('(?<![a-zA-Z0-9_$@:])[A-Z|0-9][a-zA-Z0-9\']*', data.to_string(index=False))

getFilteredWords(data)
print(len(x))
print(x)
...
def populateHashtagsList(data):
    global hashtagsList
    hashtagsList = re.findall('#[A-Za-z0-9]+', data.to_string(index=False))

def populateHashtagsMapCount():
    global hashtagsList
    global hashtagsMapCount
    
    for hashtag in hashtagsList:
        if(hashtag in hashtagsMapCount) :
            value = hashtagsMapCount[hashtag]
            value += 1
            hashtagsMapCount[hashtag] = value
        else:
            hashtagsMapCount[hashtag] = 1

def printPopularHashtags():
    global top
    global hashtagsMapCount    
    sortedHashTags = sorted(hashtagsMapCount.items(), key=lambda x: x[1], reverse=True)
    
    idx = 0
    while(idx <= top):
        print(sortedHashTags[idx])
        idx += 1
        
            
populateHashtagsList(data)
populateHashtagsMapCount()
printPopularHashtags()
...
def populateUrlsList(data):
    global urlsList
    urlsList = re.findall('http[s]*://[a-zA-Z0-9_.%/-]+', data.to_string(index=False))

populateUrlsList(data)
print(len(urlsList))
print(urlsList)
